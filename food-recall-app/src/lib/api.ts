...
import { fetchFsisRecalls, filterFsisRecalls, mergeRecallFeeds } from "./fsis";
import { mockFsisRecalls, mockRecalls } from "./mockData";
...
export async function fetchRecalls(params?: {
  ...
  const fsisPromise = fetchFsisRecalls({ signal: params?.signal }).catch(() => ({
    recalls: [],
  ...
  const withFsis = async (fdaResult: FetchResult): Promise<FetchResult> => {
    ...
    const total = (fdaResult.total || 0) + fsisCount;
    ...
    return {
      ...
      isStale: fdaResult.isStale || (fsis.isStale && includeFsis),
      fsisCount,
      fsisError: fsis.error,
    };
  };
  ...
  if (demo) {
    ...
    if (q) {
      filtered = filtered.filter((r) =>
        `${r.productDescription} ${r.reasonForRecall} ${r.recallingFirm} ${r.brand || ""} ${r.headline || ""}`
          .toLowerCase()
          .includes(q),
      );
    }
    ...
    return {
      ...
      lastSynced: getLastSynced(),
      isDemo: true,
      fsisCount: paged.filter((r) => r.source === "USDA").length,
      fsisError: null,
    };
  }
  ...
  try {
    ...
    if (!raw || typeof raw !== "object" || !Array.isArray(data.results)) {
