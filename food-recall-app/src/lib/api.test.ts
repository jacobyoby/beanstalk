...
const isFsisUrl = (url: unknown) => {
  const s = String(url);
  if (s.includes("/api/fsis") || s.includes("fsis-recalls.json") || s.includes("/api/fsis-rss/")) return true;
  try {
...
const mockSuccess = (
  ...
  vi.fn().mockImplementation(async (url: string) => {
    ...
    return {
      ...
      text: async () => JSON.stringify({ results, meta: { results: { total: results.length } } }),
    };
  });
...
describe("buildGroupedSearchClause", () => {
  ...
  it("encodes safely via encodeURIComponent (ampersand)", () => {
    ...
    const hashClause = buildGroupedSearchClause("a#b") ?? "";
    const hashEncoded = encodeURIComponent(hashClause);
...
describe("fetchRecalls — no synthetic fallback", () => {
  ...
  it("404 NOT_FOUND yields empty, not mock, no error", async () => {
    vi.stubGlobal(
      "fetch",
      mockFdaResponse({ ok: false, status: 404, body: { error: { code: "NOT_FOUND", message: "No matches found!" } } }),
    );
  ...
  it("503 server failure returns SERVER, retryable", async () => {
    vi.stubGlobal(
      "fetch",
      mockFdaResponse({ ok: false, status: 503, body: { error: { message: "Service Unavailable" } } }),
    );
    const res = await fetchRecalls({ search: "", limit: 6, skip: 0 });
  ...
  it("400 bad request returns BAD_REQUEST, not retryable", async () => {
    vi.stubGlobal(
      "fetch",
      mockFdaResponse({ ok: false, status: 400, body: { error: { message: "Invalid parameter" } } }),
    );
  ...
  it("out-of-order: second request wins, first stale ignored via caller generation (simulated)", async () => {
    ...
    const fetchMock = vi.fn().mockImplementation((url: string) => {
      ...
      if (String(url).includes("first")) return firstPromise;
      if (String(url).includes("second")) return Promise.resolve(secondBody);
    ...
    const fdaCalls = fetchMock.mock.calls.filter((call) => !isFsisUrl(call[0]));
...
describe("SPA static host fallback (#90)", () => {
  ...
  it("non-JSON 200 response (SPA shell) falls back to direct FDA URL", async () => {
    ...
    const fetchMock = vi.fn().mockImplementation(async (url: string) => {
      if (isFsisUrl(url)) return emptyFsis;
      if (String(url).includes(window.location.origin)) return htmlResponse;
    ...
    const fdaCalls = fetchMock.mock.calls.filter((call) => !isFsisUrl(call[0]));
    expect(fdaCalls).toHaveLength(2);
    expect(String(fdaCalls[1][0])).toContain("api.fda.gov/food/enforcement.json");
  });
  ...
  it("JSON parse failure returns MALFORMED, not NETWORK", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn().mockImplementation(async (url: string) => (isFsisUrl(url) ? emptyFsis : badJsonResponse)),
    );
  ...
  it("normal JSON 200 response works as before", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockImplementation(async (url: string) => (isFsisUrl(url) ? emptyFsis : goodJsonResponse)),
    );
