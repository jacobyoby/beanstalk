...
export default function RelatedEvents({ productDescription, onSelectEvent }: Props) {
  ...
  return (
    <div className="mt-6 border-t dark:border-zinc-700 pt-4">
      ...
      {loading && <p className="text-xs text-zinc-500 dark:text-zinc-400" role="status">Looking for related early signals…</p>}
      {!loading && error && <p className="text-xs text-zinc-500 dark:text-zinc-400">{error}</p>}
      {!loading && !error && events.length === 0 && (
        <p className="text-xs text-zinc-500 dark:text-zinc-400">No matching CAERS reports found for this product text.</p>
      )}
      {!loading && events.length > 0 && (
        <ul className="space-y-2">
          {events.map(ev => {
            const brand = ev.products.map(p => p.nameBrand).filter(Boolean).join(', ') || 'Unknown product'
            return (
              <li key={ev.id}>
                <button
                  type="button"
