        ...
        <dl className="mt-4 space-y-3 text-sm dark:text-zinc-300">
          <div>
            ...
            <dd>
              ...
              >
                {isUsda ? "USDA FSIS" : "FDA"}
              </span>
              <span className="ml-2 text-xs text-zinc-500 dark:text-zinc-400">
                {isUsda ? "Meat, poultry, and egg products" : "openFDA food enforcement"}
              </span>
            </dd>
          </div>
          ...
          <div>
            ...
            <dd>{recall.status}</dd>
            {!isUsda && <dd className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">{OPENFDA_AS_PUBLISHED}</dd>}
          </div>
          <div>
          ...
          {recall.establishmentNumber && (
            <div>
              <dt className="font-semibold">Establishment #</dt>
              <dd>{recall.establishmentNumber}</dd>
            </div>
          )}
          <div>
            <dt className="font-semibold">Firm</dt>
            <dd>
              {[recall.recallingFirm, [recall.city, recall.state].filter(Boolean).join(", ")]
                .filter(Boolean)
                .join(" — ")}
            </dd>
          </div>
          ...
          <div>
            ...
            <dd className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
              Source: {isUsda ? "USDA FSIS notice" : "FDA code_info"}
            </dd>
          </div>
          {recall.moreCodeInfo && (
            <div>
          ...
          {recall.productType && (
            <div>
              <dt className="font-semibold">Product Type</dt>
              <dd>{recall.productType}</dd>
            </div>
          )}
