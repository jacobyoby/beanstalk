...
for (const [, url] of html.matchAll(/(?:src|href)="([^"]+)"/g)) {
  ...
  assert.ok(url.startsWith(base), `Asset escapes the demo path: ${url}`)
...
assert.equal(manifest.name, 'Beanstalk Demo')
...
for (const icon of manifest.icons) {
  ...
  assert.ok(url.pathname.startsWith(base), `Icon escapes the demo path: ${icon.src}`)
...
async function inspect(dir) {
  for (const item of await readdir(dir, { withFileTypes: true })) {
    ...
    assert.ok(!text.includes('BEANSTALK_BUILD_SECRET_SENTINEL'), 'Demo exposed the build-time owner-key sentinel')
...
await inspect(dist)
console.log('Demo artifact verified: subpath assets, manifest, service worker, icons, and credential sentinel.')
